...
export default function CategoryCard({item, src}) {
  const link = item.secondaryCategory ? `/products/category/${item.mainCategory}/${item.secondaryCategory}` : 
        ...
        <CardBody>
          {
            ...
            <CardTitle tag='h4'>
              {categoriesNames.get(item.mainCategory)} {categoriesNames.get(item.secondaryCategory).toLowerCase()}
            </CardTitle>
            :
            <CardTitle tag='h4'>
