...
export default function Home({ title }) {
	...
	useEffect(() => {
		const fetchMainPageImages = async () => {
			const categoriesImagesUrls = await fetchImagesUrls(
				popularCategoriesItems,
			);
			...
			<section className="section-margin">
				...
				<Container className="full-width">
					<Row className="new-items__container position-relative me-0" xs="1">
						<Col
							className="d-flex flex-column new-items__tables-col"
							xs="12"
							md="10"
							lg="9"
							xl="8"
						...
						<div className="d-flex justify-content-center align-items-center new-items__image-wrap">
							<Image
								src={newItemsUrls[newItemsUrls.length - 1]}
								title={newItems[newItems.length - 1].title}
							/>
						</div>
					</Row>
				</Container>
